...
> Linux ISC DHCP server opens a raw UDP socket with the Linux kernel, bypassing the firewalld rules, so it is not necessary to allow the service/port. Be sure the resulting "/etc/dhcp/dhcpd.conf" file is only configured for the desired interface for hosting DHCP ***(enp0s8).***
> 
