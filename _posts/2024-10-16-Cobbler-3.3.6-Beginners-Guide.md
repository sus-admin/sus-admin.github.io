...
1. It's always a good idea to manually install the desired distro (Fedora 34 or 37 Server in the case of this guide) to the desired hardware before attempting a network installation, to validate the compatability.

2. Fedora installations, whether installed with *(manually)* or without *(automated)* user input, should always contain an "anaconda-ks.cfg" file located in the "/root" directory which contains a kickstart file which could be used to install a new system identical to the local system.
...
- 1. These templates use variables local to the Cobbler server to calculate/formulate a specific configuration for the desired installation, but you can always craft a specific kickstart file manually and copy it to the same `/var/lib/cobbler/templates` directory and assign a Cobbler Profile or System to use that specific file with the command: `cobbler <profile|system> edit --name <profile|system> --autoinstall <file>` **just be sure to espcape any dollar-signs ($) with back-slashes (\), otherwise they may be interpretted as variables by Cobbler**

- 2. There are several template files and accomodating snippets that come default with Cobbler which you can look at if you have any questions about how the Cobbler templates (Python Cheetah templates) work. Located in `/var/lib/cobbler/templates` & `/var/lib/cobbler/snippets`
