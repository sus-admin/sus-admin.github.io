...
2. Fedora installations, whether installed with *(manually)* or without *(automated)* user input, should always contain an "anaconda-ks.cfg" file located in the "/root" directory which is a kickstart file that could be used to install a new system identical to the local system.
