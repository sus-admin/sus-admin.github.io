...
Finally, create a new Cobbler System, replacing `aa:bb:cc:dd:ee:ff` with the Mac Address of the **PXE Client** VM created above, and syncing up Cobbler:
...
4. If using **BIOS firmware** for the PXE client VM, the Cobbler server's DHCP service may not provide the right bootloader (`filename` variable in `/etc/dhcp/dhcpd.conf` file)
...
5. The autoinstall template files "Fedora34.ks" "Fedora37.ks" & "sample.ks" located in `/var/lib/cobbler/templates` are used to dynamically generate the specific kickstart files for either a Cobbler Profile or System at the time of installation
