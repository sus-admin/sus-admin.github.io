...
sudo cobbler repo add --name bullseye-latest --keep-updated false --mirror-locally false --breed apt --arch x86_64 --mirror "http://deb.debian.org/debian" --apt-components="main non-free non-free-firmware contrib" --apt-dists=bullseye
sudo cobbler repo add --name bullseye-updates --keep-updated false --mirror-locally false --breed apt --arch x86_64 --mirror "http://deb.debian.org/debian" --apt-components="main non-free non-free-firmware contrib" --apt-dists=bullseye-updates
sudo cobbler repo add --name bullseye-security --keep-updated false --mirror-locally false --breed apt --arch x86_64 --mirror "http://security.debian.org/debian-security" --apt-components="main non-free non-free-firmware contrib" --apt-dists=bullseye-security
```
...
Modify the new preseed (autoinstall) template to deselect any services, install desired packages only, setup an additional user "debian" with `sudo` privileges, set the workaround script created above to run in the "early" installation stage, and the "preseed_late" script to run in the late stage, and more aggressively force the installation regardless of any existing installations.
...
Add a Cobbler Distro and Profile (including the metadata-only `cobbler repo` mirrors created above so the system will have the latest official package sources once installed) for the new Debian repo, and the usual HTTP link:
...
1. The `debian-installer` issue mentioned in the **Caveats** section above is due to the `base-installer` [source code](https://salsa.debian.org/installer-team/base-installer/-/blob/1.213/library.sh?ref_type=tags) neglacting to add the `Acquire::AllowInsecureRepositories "true";` line in to the `00AllowUnauthenticated` file on the target system (created depending on the value of the `debian-installer/allow_unauthenticated` preseed value) as mentioned in the [apt-secure manpage](https://manpages.debian.org/bookworm/apt/apt-secure.8.en.html).
