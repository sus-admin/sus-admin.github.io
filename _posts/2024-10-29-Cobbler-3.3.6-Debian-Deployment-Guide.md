...
1. The [Cobbler 3.3.6 Beginner's Guide](2024-10-16-Cobbler-3.3.6-Beginners-Guide.md) **Tips & Troubleshooting** section contains some basic recommendations and known limitations of Cobbler which will not be repeated here, so be sure to check there as well.

1. Regaring the issue mentioned in the **Caveats** section above, here is a description of the issue, without getting overly detailed about all the troubleshooting and alternative strategies used when attempting to resolve. 

    - The issue is first encounted after the PXE client loads the `debian-installer` correctly and retrieved the appropriate preseed file from the Cobbler HTTP server. Some time into the installation, if workaround script (`preseed_early_workaround`) created above is not configured in the preseed file, the PXE client will begin the `apt-setup` phase to begin installing packages from the repo mirror, but fails on a fatal error, warning that the repo mirror is not trusted.

        - The error prevents the installation from proceeding using the configured repo mirror, but the `debian-installer` remains running, and you can press **ALT+F2** or **ALT+F3** on the PXE client keyboard to access a privileged shell to perform some troubleshooting.
    ...
    - As seen in the procdures above, `allow_unauthenticate` is set to true, which appears to only permit the installation to START when an untrusted repo mirror is found, but the mirror will be checked again before beginning the `apt-setup` phase, as described above, so that option is not fully sufficient to complete a full Debian OS installation from a unofficial Debian mirror (also hinted at by the comment in the preseed snippet above: "you must provide a key or apt will complain about the unauthenticated repository").

        - This means that in addition to the `allow_unauthenticated` preseed option, the `debian-installer` also requires the local APT mirror to be defined using the `d-i apt-setup/local0/repository` & `d-i apt-setup/local0/key` preseed options in order for the PXE client to retrieve the public GPG key for the repo mirror and use it to validate the mirror, which I belive has been configured properly in the procedures above (and obviously can be done, since the workaround provided in this guide does essentially the same thing).

    - The reason I believe this is a bug is that the `preseed-early-workaround` script uses the same logic and configurations that the `apt-setup/local0` preseed options should impose on the `debian-installer` in order to trust the APT source, but the native preseed function will fail to download the public GPG key from the Cobbler server to the PXE client.

        - This failure to download can be seen in `/var/log/syslog` on the PXE client once the installation has failed, in a log entry similar to: `WIP`
    ...
    - The issue is present with both Debian 11 & 12
